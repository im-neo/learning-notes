...
```shell
[root@SnailClimb local]#wget http://erlang.org/download/otp_src_19.3.tar.gzCopy to clipboardErrorCopied
```
...
```shell
[root@SnailClimb local]#tar -xvzf otp_src_19.3.tar.gzCopy to clipboardErrorCopied
```

**3 删除 erlang 安装包**

```shell
[root@SnailClimb local]#rm -rf otp_src_19.3.tar.gzCopy to clipboardErrorCopied
```

**4 安装 erlang 的依赖工具**

```shell
[root@SnailClimb local]#yum -y install make gcc gcc-c++ kernel-devel m4 ncurses-devel openssl-devel unixODBC-develCopy to clipboardErrorCopied
```
...
```shell
[root@SnailClimb local]# mkdir erlangCopy to clipboardErrorCopied
```

对 erlang 进行安装环境的配置

```shell
# ./configure --prefix=/usr/local/erlang --without-javacCopy to clipboardErrorCopied
```
...
```shell
[root@SnailClimb otp_src_19.3]# ./bin/erlCopy to clipboardErrorCopied
```

运行下面的语句输出“hello world”

```erlang
 io:format("hello world~n", []).Copy to clipboardErrorCopied
```

![输出“hello world”](images/49570541.jpg)
...
```shell
[root@SnailClimb etc]# vim profileCopy to clipboardErrorCopied
```
...
export ERL_HOME PATHCopy to clipboardErrorCopied
```

运行下列命令使配置文件`profile`生效

```shell
[root@SnailClimb etc]# source /etc/profileCopy to clipboardErrorCopied
```

输入 erl 查看 erlang 环境变量是否配置正确

```shell
[root@SnailClimb etc]# erlCopy to clipboardErrorCopied
```

![输入 erl 查看 erlang 环境变量是否配置正确](images/62504246.jpg)
...
```shell
wget https://www.rabbitmq.com/releases/rabbitmq-server/v3.6.8/rabbitmq-server-3.6.8-1.el7.noarch.rpmCopy to clipboardErrorCopied
```
...
```shell
rpm --import https://www.rabbitmq.com/rabbitmq-release-signing-key.ascCopy to clipboardErrorCopied
```

紧接着执行：

```shell
yum install rabbitmq-server-3.6.8-1.el7.noarch.rpmCopy to clipboardErrorCopied
```
...
```shell
chkconfig rabbitmq-server onCopy to clipboardErrorCopied
```

**4. 启动服务**

```shell
service rabbitmq-server startCopy to clipboardErrorCopied
```

**5. 查看服务状态**

```shell
service rabbitmq-server statusCopy to clipboardErrorCopied
```
...
```shell
“guest” user can only connect via localhostCopy to clipboardErrorCopied
```
...
再次访问http://127.0.0.1:15672/ ,输入用户名和密码：root root

![RabbitMQ控制台](images/45835332.jpg)